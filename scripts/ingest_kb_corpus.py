...
def collect_files(limit: int = 0, only: List[str] | None = None) -> List[Tuple[Path, str]]:
    ...
    if only:
        ...
        files = [f for f in files if f[1] in only_set or any(o in f[1] for o in only_set)]
...
def split_all(files: List[Tuple[Path, str]]) -> List[Document]:
    ...
    from backend.utils.splitter import split_markdown
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="只入库指定文件 (sop 文件名如 disk_cleanup_sop.md，或 kb_corpus 相对路径)",
    )
    ...
    files = collect_files(limit=args.limit, only=args.only)
