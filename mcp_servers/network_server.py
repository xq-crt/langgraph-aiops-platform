...
提供本机网络排查能力, 让 OnCall Agent 能真实诊断 "网站打不开 / DNS 异常 / 端口不通" 等问题:
  - net_ping: 测试主机连通性 + 丢包率 + 延迟
  - net_http_probe: HTTP/HTTPS 健康检查 (状态码 + 响应时间)
  - net_dns_resolve: 域名 DNS 解析
  - net_tcp_port_check: TCP 端口可达性
...
def net_http_probe(url: str, timeout_sec: float = 10.0) -> str:
    url = (url or "").strip()
    if not url:
        return "[拒绝] url 不能为空"
    ...
    if _is_blocked_ip(host):
        return f"[拒绝] {host} 是内网/回环地址, 不允许 net_http_probe"
...
def net_dns_resolve(domain: str) -> str:
...
@mcp.tool(
    name="net_tcp_port_check",
    description=(
        "检测指定 host 的 TCP 端口是否可达. 用于排查 '服务起来了吗 / 防火墙挡了吗'. "
        "拒绝扫描内网 IP. timeout 默认 3 秒."
    ),
)
def net_tcp_port_check(host: str, port: int, timeout_sec: float = 3.0) -> str:
    host = (host or "").strip()
