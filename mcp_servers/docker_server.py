...
提供本机 Docker 容器的查询能力, 用于诊断 "Milvus / Redis / 数据库容器是不是挂了":
  - container_list: 列出所有容器及状态
  - container_resource_stats: 容器资源占用 (CPU/MEM)
  - container_logs_tail: 拉取容器最近日志
  - container_config_inspect: 容器详细信息
  - container_restart: 重启容器 (写操作, 默认禁用, 需明确开启)

底层: subprocess 调用 docker CLI, 无需 docker-py 依赖.
读模式: 默认只读. container_restart 通过 DOCKER_ALLOW_RESTART=true 开启.
"""
...
@mcp.tool(
    name="container_resource_stats",
    description=(
        "查看一个容器的实时资源占用 (CPU% / 内存使用 / 网络IO / 磁盘IO). "
        "name 为容器名 (从 container_list 拿到). 一次性快照, 不持续监控."
    ),
)
def container_resource_stats(name: str) -> str:
    name = (name or "").strip()
    if not name:
        return "[拒绝] name 不能为空"
...
@mcp.tool(
    name="container_config_inspect",
    description=(
        "查看容器的详细配置 (镜像 / 启动命令 / 挂载卷 / 网络模式 / 重启策略). "
        "用于排查 '为什么这个容器一直重启' 等深度问题."
    ),
)
def container_config_inspect(name: str) -> str:
    name = (name or "").strip()
    if not name:
        return "[拒绝] name 不能为空"
...
@mcp.tool(
    ...
    description=(
        ...
        "用于挂掉的容器恢复. 调用前最好先 container_logs_tail 看下原因."
    ),
)
def container_restart(name: str) -> str:
    if not DOCKER_ALLOW_RESTART:
        return (
            "[拒绝] container_restart 是危险操作, 默认禁用. "
