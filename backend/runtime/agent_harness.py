...
3. 只完成当前步骤, 不要越界规划下一步。
4. 若当前步骤要求查知识库: 调用 kb_search_sop 时 query 必须包含用户问题中的故障域关键词
   (如磁盘/空间不足、CPU/内存、网络/Docker、Redis/MySQL 等); 若返回片段来源与故障域明显不符
   (例如磁盘问题却全是 HTTP 5xx/RAIL), 应换更具体的关键词重搜, 或明确说明未命中相关 SOP。
5. 始终使用中文输出。
...
# 新摘要"""
    ...
    def planner_fallback_plan(self, reason: str, skill_name: str = "") -> list[str]:
        """Planner 失败或空计划时的默认步骤 (按 Skill 区分, 避免磁盘场景先泛化查 KB)."""
        skill = (skill_name or "").strip()
        conclude = "汇总诊断结论, 输出根因与处置建议"

        if reason == "empty_plan":
            return [conclude]

        if skill == "disk_cleanup_diagnosis":
            return [
                "调用 host_disk_partitions 查看各分区使用率与剩余空间",
                "调用 host_scan_dir_usage 扫描占用最高的目录 TopN",
                "汇总诊断结论, 给出可清理项与人工确认步骤",
            ]

        if skill == "host_resource_diagnosis":
            return [
                "调用 host_snapshot 获取 CPU/内存/磁盘整体快照",
                "根据快照结果调用 host_top_processes 或 host_disk_partitions 深入排查",
                conclude,
            ]

        if skill == "container_diagnosis":
            return [
                "调用 container_list 与 container_resource_stats 查看容器状态",
                "调用 container_logs_tail 查看异常容器最近日志",
                conclude,
            ]

        if skill == "network_diagnosis":
            return [
                "调用 net_ping 与 net_http_probe 检查连通性与 HTTP 状态",
                "必要时调用 net_dns_resolve 检查 DNS 解析",
                conclude,
            ]

        kb_step = (
            "调用 kb_search_sop, query 须包含与用户问题一致的故障域关键词"
            " (磁盘/CPU/网络/容器/中间件等), 禁止只搜「类似问题经验」"
        )
        return [kb_step, conclude]
