...
def _get_high_risk_set() -> Set[str]:
    from backend.runtime.tool_filter import HIGH_RISK_TOOLS

    return HIGH_RISK_TOOLS


def _get_notification_set() -> Set[str]:
    from backend.runtime.tool_filter import NOTIFICATION_TOOLS
...
def evaluate_permission(
    ...
    *,
    ...
    # 允许任何 Skill 下使用. 动机:
    #   - 诊断类 Skill 经常漏写某个本机指标工具 (如 host_top_processes), 会导致 Agent
    ...
    # TODO: 支持 "Bash(git *)" / "container_restart(container=nginx)" 这类规则
