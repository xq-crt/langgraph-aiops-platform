...
调用方:
  - backend.services.rag_service: 走结构化 hits, 自己拼提示
...
def get_provider() -> str:
    ...
    try:
        ...
        val = settings.net_web_search_provider
    except Exception:
    ...
    return [
        {
            ...
            "snippet": (
                f"这是 net_web_search 的 Mock 占位返回 (provider=mock). "
...
def search(query: str, max_results: int, provider: str | None = None) -> List[Dict[str, Any]]:
    ...
    raise ValueError(f"未知 net_web_search provider: {p!r} (可选 open_websearch / mock / ddgs)")
