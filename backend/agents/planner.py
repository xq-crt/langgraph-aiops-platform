...
from backend.agents.state import Plan, PlanExecuteState
from backend.core.llm import get_chat_llm
from backend.core.structured import ainvoke_structured
...
async def plan_node(state: PlanExecuteState) -> PlanExecuteState:
    ...
    try:
        ...
        return {
            "plan": harness.planner_fallback_plan("llm_failed", skill.name),
            "iteration": 0,
    ...
    if not plan.steps:
        ...
        return {
            "plan": harness.planner_fallback_plan("empty_plan", skill.name),
            "iteration": 0,
