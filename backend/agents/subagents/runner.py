...
主 Executor 通过 ainvoke({"name": "subagent_collect_evidence",
                          "args": {"task": "..."}}) 来委托.
...
from backend.agents.subagents import SUBAGENTS, SubagentDefinition

# Breaking rename: delegate_to_* -> subagent_* (portfolio phase C)
SUBAGENT_TOOL_NAMES: dict[str, str] = {
    "evidence_collector": "subagent_collect_evidence",
    "kb_researcher": "subagent_research_kb",
    "report_writer": "subagent_draft_report",
}
from backend.core.llm import get_chat_llm
from backend.runtime.permissions import PermissionDecision
...
async def _run_subagent(sub: SubagentDefinition, task: str) -> str:
    ...
    from backend.tools.mcp_loader import get_all_tools as _get_all_tools
...
def _make_delegate_tool(sub: SubagentDefinition) -> BaseTool:
    ...
    return StructuredTool.from_function(
        ...
        name=SUBAGENT_TOOL_NAMES.get(sub.agent_type, f"subagent_{sub.agent_type}"),
