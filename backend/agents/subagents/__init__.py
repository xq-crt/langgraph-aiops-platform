...
Split the main OnCall executor into three focused helpers:
  ...
  | Subagent           | 职责                          | 工具池                                    |
  |--------------------|-------------------------------|-------------------------------------------|
  | evidence_collector | 只读证据收集 (指标/日志/进程)  | get_local_*, query_*, host_top_processes |
  | kb_researcher      | 本地 KB + 联网兜底搜索         | kb_search_sop, net_web_search         |
...
SUBAGENTS: dict[str, SubagentDefinition] = {
    ...
    "kb_researcher": SubagentDefinition(
        ...
        description=(
            ...
            "输入参数: task (要查什么). "
            "**优先本地知识库, 不够才允许联网, net_web_search 最多 2 次**."
        ),
        system_prompt=(
            "你是一个知识检索 Agent.\n"
            "策略:\n"
            "  1. 先调 kb_search_sop, 命中就用本地结果\n"
            "  2. 本地不够 / 主题陌生才用 net_web_search, 最多 2 次, 否则成本失控\n"
            "  3. 返回 Markdown 列表, 每条注明来源 (kb / web)\n"
            "  4. **只整理引用, 不要自己凭印象续写经验**"
        ),
        allowed_tools=["kb_search_sop", "net_web_search"],
        max_iters=3,
