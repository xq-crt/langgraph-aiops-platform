...
class ToolMeta(BaseModel):
    ...
    is_notification: bool = Field(
        ...
        description=(
            ...
            "例如 mcp_invoke_by_name 也是 external 但不是通知."
        ),
    )
...
TOOL_META: Dict[str, ToolMeta] = {
    ...
    "host_scan_dir_usage": ToolMeta(
        read_only=True,
        concurrency_safe=False,
        side_effect="filesystem",
        max_result_chars=8000,
        risk_level="low",
        search_hint="disk directory scan usage 目录占用",
    ),
    "host_find_large_files": ToolMeta(
        read_only=True,
        concurrency_safe=False,
        side_effect="filesystem",
        max_result_chars=12000,
        risk_level="low",
        search_hint="large files disk 大文件",
    ),
    "host_cleanup_hints": ToolMeta(
        read_only=True,
        concurrency_safe=True,
        max_result_chars=4000,
        risk_level="low",
        search_hint="disk cleanup hints 清理建议",
    ),

    # ===== Windows 事件日志 (winlog_server.py) =====
    "win_events_query": ToolMeta(
        read_only=True,
        concurrency_safe=True,
    ...
    # 只读 container_list / container_resource_stats / container_logs_tail / container_config_inspect 都是 low.
    # container_restart 是写操作, high.
    "container_list": ToolMeta(
        read_only=True,
        concurrency_safe=True,
    ...
    "container_resource_stats": ToolMeta(
        read_only=True,
        concurrency_safe=True,
        max_result_chars=2000,
        risk_level="low",
    ...
    "container_config_inspect": ToolMeta(
        read_only=True,
        concurrency_safe=True,
        max_result_chars=8000,
        risk_level="low",
    ...
    # ===== Lazy MCP 元工具 =====
    # mcp_discover_tools 是只读的工具发现入口, 可并发.
    # mcp_invoke_by_name 是动态执行入口, 由它内部再做安全决策, 这里保守不并发.
    "mcp_discover_tools": ToolMeta(
        read_only=True,
        concurrency_safe=True,
        max_result_chars=4000,
        risk_level="low",
    ...
    # 多个 subagent_collect_evidence / kb_researcher 同时进可并行 (各自子上下文独立).
    "subagent_collect_evidence": ToolMeta(
        read_only=True,
        concurrency_safe=True,
        risk_level="low",
        max_result_chars=8000,
    ...
    "subagent_draft_report": ToolMeta(
        read_only=True,
...
def warn_unregistered_tools(tool_names: list[str]) -> list[str]:
    ...
    if missing:
        logger.warning(
            ...
            f"{sorted(missing)}. 请在 backend/tools/meta.py 补登记, 否则会影响并行编排和 PermissionMode 决策."
        )
    return missing
