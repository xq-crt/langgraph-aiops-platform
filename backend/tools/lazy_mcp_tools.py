...
from backend.core.mcp_client import mcp_client_manager
...
def create_lazy_mcp_tools(allowed_tool_names: Set[str]) -> List[BaseTool]:
    ...
    async def mcp_discover_tools(query: str = "") -> str:
        """搜索当前 Skill 允许调用的 MCP 工具, 用于按需发现远程工具能力.

        Args:
            query: ?????, ?? docker???????net_web_search?
    ...
    async def mcp_invoke_by_name(tool_name: str, arguments_json: str = "{}") -> str:
        """按名称执行当前 Skill 允许的 MCP 工具.

        Args:
            tool_name: 要执行的 MCP 工具名, 必须先通过 mcp_discover_tools 发现。
        ...
        if tool is None:
            return f"未找到 MCP 工具: {tool_name}。请先调用 mcp_discover_tools 确认可用工具。"
    ...
    return [
        StructuredTool.from_function(
            coroutine=mcp_discover_tools,
            name="mcp_discover_tools",
            description="搜索当前 Skill 允许的 MCP 工具。先用它发现工具名和用途, 再用 mcp_invoke_by_name 执行。",
        ),
        StructuredTool.from_function(
            coroutine=mcp_invoke_by_name,
            name="mcp_invoke_by_name",
            description="按名称执行当前 Skill 允许的 MCP 工具。tool_name 必须来自 mcp_discover_tools, arguments_json 必须是 JSON object 字符串。",
        ),
    ]
