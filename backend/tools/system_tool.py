...
def host_snapshot() -> str:
    ...
    try:
        ...
        logger.exception(f"[system_tool] host_snapshot 失败: {e}")
...
def host_cpu_memory() -> str:
    ...
    try:
        ...
        logger.exception(f"[system_tool] host_cpu_memory 失败: {e}")
...
def host_disk_partitions() -> str:
    ...
    try:
        ...
        logger.exception(f"[system_tool] host_disk_partitions 失败: {e}")
...
def host_top_processes(sort_by: str = "memory", limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception(f"[system_tool] host_top_processes 失败: {e}")
...
def host_scan_dir_usage(
    ...
    """仅采集运行本服务的主机目录占用 TopN（非 SSH 远程）。只读，不删除文件。"""
    ...
    try:
        ...
        if truncated:
            lines.append("", "*扫描因超时或深度限制未完全结束。*")
        return "\n".join(lines)
    except Exception as e:
        return f"目录扫描失败: {type(e).__name__}: {e}"
...
def host_find_large_files(
    ...
    try:
        ...
        if not rows:
            lines.append("| （无匹配） | — |")
        if truncated:
            lines.append("", "*扫描未完全结束。*")
        return "\n".join(lines)
    except Exception as e:
        return f"大文件扫描失败: {type(e).__name__}: {e}"
