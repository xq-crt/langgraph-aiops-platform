...
from backend.tools.knowledge_tool import kb_search_sop
from backend.tools.system_tool import (
    host_cleanup_hints,
    host_cpu_memory,
...
from backend.tools.time_tool import clock_now


def get_local_tools() -> List[BaseTool]:
    """返回所有本地 @tool 工具.

    注: net_web_search 仍由 MCP server (mcp_servers/websearch_server.py) 提供.
...
def get_all_tools() -> List[BaseTool]:
    ...
    打 warning, 帮助开发者及时补 backend/tools/meta.py.

    Returns:
        合并后的工具列表
    """
    # 延迟 import 避免循环 (meta -> tools.__init__ -> mcp_loader)
    from backend.tools.meta import warn_unregistered_tools

    # §5: subagent delegate 工具 (subagent_collect_evidence 等)
    # 延迟 import: subagents.runner 内部依赖 mcp_loader, 避免循环
    from backend.agents.subagents.runner import get_subagent_tools
