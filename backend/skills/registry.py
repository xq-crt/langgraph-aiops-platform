...
设计:
  - 文件布局: backend/skills/definitions/<skill_name>/SKILL.md
...
from backend.skills.loader import SkillLoadError, load_skill_from_file
from backend.skills.models import Skill

# Skill 定义目录: backend/skills/definitions/<skill_name>/SKILL.md
    ...
    def get_or_generic(self, name: Optional[str]) -> Skill:
        ...
        if generic is None:
            raise RuntimeError(
                ...
                f"请确认 backend/skills/definitions/{GENERIC_SKILL_NAME}/SKILL.md 存在"
            )
        return generic
