...
## Phase 1: 摸底 (容器存在吗?)
1. 调 `container_list()` 看本机所有容器列表 + 状态
2. 在结果里找用户提到的容器名
3. 状态判断:
...
## Phase 2: 健康度 (在跑但表现差?)
1. 调 `container_resource_stats(name)` 看实时 CPU/MEM/IO
2. 阈值参考:
   - CPU > 90% 持续 → 容器内应用 CPU 密集 / 死循环
   - MEM 接近 limit → 即将 OOMKilled
   - 网络 IO 异常高 → 网络热点
3. 调 `container_logs_tail(name, tail=100)` 看最近日志:
   - 找 `ERROR` / `FATAL` / `panic` / `OOM` 关键字
   - 看时间戳是否密集 (高频报错)

## Phase 3: 死亡分析 (停止/重启循环)
1. 调 `container_config_inspect(name)` 拿:
   - `State.ExitCode` — 退出码:
     - 0 = 正常退出 (但可能不是预期)
...
2. 调 `container_logs_tail(name, tail=200, since_minutes=10)` 看死前最后日志
3. 推断:
...
- **临时止血**: 如果 `DOCKER_ALLOW_RESTART=true` 已配置, 可调 `container_restart(name)` 拉起容器, 但**先看日志** 确认重启不会丢数据

## 注意事项
- **container_restart 默认禁用** — 是写操作, 需 `.env` 显式开启 `DOCKER_ALLOW_RESTART=true`
