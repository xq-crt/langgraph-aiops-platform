...
### 第 1 层 — DNS 解析
- 调 `net_dns_resolve(domain)` 看域名能否解析到 IP
- 解析失败 → 大概率本机 DNS 配置问题或域名失效

### 第 2 层 — 网络连通性
- 调 `net_ping(host)` 看主机能否通达
- 不通 → 可能 ICMP 被禁 / 路由问题 / 主机宕机
- 通但延迟高 → 网络拥塞或跨地域

### 第 3 层 — 端口可达性
- 调 `net_tcp_port_check(host, port)` 验证目标端口
- 拒绝 → 服务未监听 (进程没起来)
- 超时 → 防火墙挡了 / 安全组没开

### 第 4 层 — 应用层
- 调 `net_http_probe(url)` 发真实 HTTP 请求
...
- **限频** — 单次诊断 ping 不超过 10 次, net_http_probe 不超过 3 个 URL
