...
class Settings(BaseSettings):
    ...
    # ==================== 应用基础 ====================
    app_name: str = Field(default="LangGraphAIOps", description="应用名")
    ...
    milvus_collection: str = Field(default="langgraph_aiops_kb", description="Collection 名")
    ...
    rag_chat_net_web_search_enabled: bool = Field(default=False, description="是否允许 RAG Chat 使用受限联网搜索")
    rag_chat_net_web_search_max_results: int = Field(default=3, description="RAG Chat 联网搜索最大结果数")
    rag_chat_net_web_search_keywords: str = Field(
        default=(
    ...
    docker_allow_restart: bool = Field(
        default=False,
        description="是否允许 Agent 调用 container_restart (危险写操作, 默认禁用)",
    )
