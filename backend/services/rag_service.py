...
仅负责把各子模块拼成 SSE 事件流, 具体逻辑在:
  - backend.services.rag.retrieval     知识库检索 + context 拼接
  - backend.services.rag.web_context   联网搜索 + 安全过滤
  - backend.services.rag.memory        query 改写 + 历史压缩
  - backend.runtime.agent_harness      Prompt / 轮次 / 降级策略
  - backend.services.rag.utils         消息格式化辅助
"""
...
from backend.agents.stream_sink import set_sink
from backend.config import settings
from backend.core.web_search import get_provider as get_net_web_search_provider
from backend.core.llm import get_chat_llm
from backend.runtime.agent_harness import HarnessUsageStats, get_agent_harness
from backend.runtime.tool_runner import run_parallel_agent
import backend.services.chat_memory as chat_memory
from backend.services.rag.memory import compact_if_needed, rewrite_question
from backend.services.rag.retrieval import build_context
from backend.services.rag.utils import content_to_text, history_to_messages
from backend.services.rag.web_context import build_web_context
from backend.tools.mcp_loader import get_all_tools
from backend.tools.meta import get_meta


# RAG chat 不应该让 LLM 调诊断专用工具 / 写工具 / 元工具,
# 只暴露"看一眼系统状态"这种纯只读 + 用户可理解的工具.
_RAG_TOOL_EXCLUDE = {
    "kb_search_sop",  # 已经在前置 retrieve 阶段做了, 不让 LLM 重复调
    "net_web_search",             # 联网走前端开关, 不让 LLM 自己决定 (避免乱搜)
    "clock_now",       # 没必要
    "mcp_discover_tools",       # Lazy MCP 元工具
    "mcp_invoke_by_name",
    "subagent_collect_evidence",   # 诊断专用 subagent
    "subagent_research_kb",
    "subagent_draft_report",
}
...
async def stream_chat(
    ...
    *,
    ...
    logger.info(
        ...
        f"top_k={top_k} | net_web_search={net_web_search} | mcp_tools={mcp_tools}"
    )
    ...
    # ---------- Stage 2: retrieve + net_web_search 并行 ----------
    ...
    if net_web_search:
        yield progress("web", "正在联网补充资料", "", data={"provider": get_net_web_search_provider()})
        ...
        if web_hits:
            yield progress(
                ...
                data={
                    ...
                    "provider": get_net_web_search_provider(),
                },
            )
        else:
            yield progress(
