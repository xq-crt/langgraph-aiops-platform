...
策略:
  ...
  - 命中黑名单 / 敏感词直接拒
  - 真正的 provider 调度走 backend.core.web_search
"""
...
from backend.core.web_search import format_results, get_provider, search
...
def _allowed_web_keywords() -> list[str]:
    return [
        item.strip().lower()
        for item in settings.rag_chat_net_web_search_keywords.split(",")
        if item.strip()
    ]
...
async def build_web_context(
    ...
    *,
    ...
    if not settings.rag_chat_net_web_search_enabled:
    ...
    max_results = max(1, min(settings.rag_chat_net_web_search_max_results, 5))
    logger.info(
